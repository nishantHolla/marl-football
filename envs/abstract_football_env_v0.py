...
class AbstractFootballEnv_V0(ParallelEnv):
	...
	def __init__(self, max_cycles=5, num_of_agents=2, field_width=800, render_mode="none"):
		...
		self.observation_spaces = {
			agent: Box(low=0, high=max(self.field_width, self.field_height), shape=(8 + 2 * self.num_of_agents,), dtype=np.float32)
	...
	def step(self, actions):
		...
		self.previous_positions = {agent: self.positions[agent].copy() for agent in self.agents}
		...
		if np.linalg.norm(goal_dir) > 0:
			goal_alignment = np.dot(ball_movement, goal_dir) / (np.linalg.norm(goal_dir) * np.linalg.norm(ball_movement) + 1e-8)
		else:
			goal_alignment = 0
		...
		for agent in self.agents:
			...
			self.ball_reward[agent] = 0.1 if self.previous_ball[0] > self.ball[0] else 0.0
			self.goal_reward[agent] = 0.1 * goal_alignment

			self.rewards[agent] = self.distance_reward[agent] + \
	...
	def _observe(self, agent):
		norm_pos = self.positions[agent] / np.array([self.field_width, self.field_height])
		...
		norm_goal_dist = np.linalg.norm(self.goal_position - self.ball) / self.field_width
		norm_ball_dist = np.linalg.norm(self.positions[agent] - self.ball) / self.field_width
